...
import JoernWrapper.{JoernService, JoernServiceImpl}
...
class CodeSliceImp(inputDir : String, outputDir: String) extends CodeSlice {
  ...
  private val joernService: JoernService = JoernServiceImpl
  val cpg: Cpg = joernService.createCpg(inputPath = inputDir, enableTsTypes = false).getOrElse(
    throw new RuntimeException("Fail to get CPG")
  )
